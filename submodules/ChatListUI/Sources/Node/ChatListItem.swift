...
public class ChatListItemNode: ItemListRevealOptionsItemNode {
    ...
    var exteraIconView: ComponentHostView<Empty>?
    var exteraIconComponent: EmojiStatusComponent?
    ...
    func asyncLayout() -> (_ item: ChatListItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool, _ nextIsPinned: Bool, _ nextHasActiveRevealControls: Bool) -> (ListViewItemNodeLayout, (Bool, Bool) -> Void) {
        ...
        return { item, params, first, last, firstWithHeader, nextIsPinned, nextHasActiveRevealControls in
            ...
            var currentExteraIconContent: EmojiStatusComponent.Content?
                ...
                if displayAsMessage {
                    ...
                    case let .peer(peerData):
                        ...
                        if let peer = iconPeer {
                            ...
                            // Shadow uses the before-name slot; exteraGram has its own icon.
                            ...
                            if let exteraEmojiId = ayuExteraBadgeEmojiId(peerId: peer.id) {
                                currentExteraIconContent = .animation(content: .customEmoji(fileId: exteraEmojiId), size: CGSize(width: 32.0, height: 32.0), placeholderColor: item.presentationData.theme.list.mediaPlaceholderColor, themeColor: item.presentationData.theme.list.itemAccentColor, loopMode: .count(0))
                            }
                        }
                    default:
                        break
            ...
            if currentExteraIconContent != nil {
...
//                    strongSelf.contextContainer.position = contextContainerFrame.center
                    ...
                    if let currentExteraIconContent {
                        ...
                        if let current = strongSelf.exteraIconView {
                            ...
                            strongSelf.mainContentContainerNode.view.addSubview(exteraIconView)
                        }
                        ...
                        let exteraIconComponent = EmojiStatusComponent(
                            ...
                            animationCache: item.interaction.animationCache,
                            animationRenderer: item.interaction.animationRenderer,
                            content: currentExteraIconContent,
                            isVisibleForAnimations: strongSelf.visibilityStatus && item.context.sharedContext.energyUsageSettings.loopEmoji,
                            action: nil
                        )
                        ...
                        transition.updateFrame(view: exteraIconView, frame: CGRect(origin: CGPoint(x: nextTitleIconOrigin, y: floorToScreenPixels(titleFrame.maxY - lastLineRect.height * 0.5 - iconSize.height / 2.0) - UIScreenPixel), size: iconSize))
