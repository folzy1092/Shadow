        ...
        |> mapToSignal { inputPeer -> Signal<PeerReadState, PeerReadStateValidationError> in
            ...
            case let .inputPeerChannel(inputPeerChannelData):
                ...
                case let .idBased(maxIncomingReadId, _, _, _, markedUnread):
                    // AyuGram: when hiding read receipts, don't send the read to the
                    ...
                    |> mapToSignal { hideReadReceipts -> Signal<Void, NoError> in
                        ...
                        return network.request(Api.functions.channels.readHistory(channel: Api.InputChannel.inputChannel(.init(channelId: channelId, accessHash: accessHash)), maxId: maxIncomingReadId))
