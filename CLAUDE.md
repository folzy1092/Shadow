...
## AyuGram edit/delete marker icons

`ChatMessageDateAndStatusNode.swift`'s `ayuEditedIcon`/`ayuDeletedIcon` (`ASImageNode`s, positioned like the pre-existing `impressionIcon`) draw AyuGram Desktop's pencil/trash icons next to the message date. Two non-obvious pitfalls, both cost a long debugging session (commit `0209efc0eb`):

- **The upstream AyuGram Desktop PNGs are not transparent silhouettes.** `Chat/AyuGram/Edited`/`Deleted` in `Images.xcassets` originally came from `raw.githubusercontent.com/AyuGram/AyuGramDesktop/dev/Telegram/Resources/icons/ayu/{edited,trash_bin}.png` — every pixel in those files has alpha=255 (a solid black square with a white glyph baked in; AyuGram Desktop is fine with that, it draws them with `useIconColor: false`, i.e. uses the baked color as-is, at their native 20x24px next to its 13px `msgDateFont`). Any masking/tinting technique applied directly to the raw upstream file renders a solid box, not a silhouette.
- **No runtime tint mechanism was found that actually works for this node.** `ASDisplayNode.tintColor` (`ASDisplayNode+UIViewBridge.mm`) is guarded by `ASDisplayNodeAssert(!_flags.layerBacked, ...)` and silently no-ops on `isLayerBacked = true` nodes (which these icon nodes are, matching their siblings). `ASImageNode.customTintColor` (Display's Swift wrapper, `submodules/Display/Source/Nodes/ASImageNode.swift`) sets `layer.layerTintColor` directly and looked like the fix, but did **not** tint this node's raw `contents = image.cgImage` assignment either — icons rendered pure black regardless. **Fix that actually shipped:** the xcassets PNGs are regenerated from the pristine upstream files with the intended gray baked directly into the RGB channels (alpha = source luminance, RGB = flat gray), so the asset needs no tinting at all — plain `image`, no `.alwaysTemplate`, no tint call. If re-syncing icons from upstream, or wanting a theme-adaptive color, redo that bake (or switch to `generateTintedImage`'s manual `context.clip(to:mask:)`, `submodules/Display/Source/GenerateImage.swift:302` — untested here for this node, but doesn't depend on either tint API above).
